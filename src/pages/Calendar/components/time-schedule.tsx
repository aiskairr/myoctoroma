...
import { Plus, X, Clock, User, Calendar, GripVertical, RotateCcw } from 'lucide-react';
...
const AdvancedScheduleComponent: React.FC = () => {
    ...
    const isTimeSlotOccupied = useCallback((employeeId: string, timeSlot: string): boolean => {
        const slotMinutes = timeToMinutes(timeSlot);
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        ...
        if (dragState.isDragging && dragState.draggedAppointment) {
            // Ограничиваем перемещение только внутри колонки текущего пользователя
            const currentEmployeeId = dragState.draggedAppointment.employeeId;
            const timeSlotIndex = Math.max(0, Math.min(Math.floor((mouseY - HEADER_HEIGHT) / TIME_SLOT_HEIGHT), timeSlots.length - 1));
            const targetTimeSlot = timeSlots[timeSlotIndex];

            // Обновляем состояние только при изменении временного слота
            if (targetTimeSlot && dragState.targetSlot?.timeSlot !== targetTimeSlot) {
                setDragState(prev => ({
                    ...prev,
                    currentPosition: { x: e.clientX, y: e.clientY },
                    targetSlot: { employeeId: currentEmployeeId, timeSlot: targetTimeSlot }
                }));
            } else {
                // Просто обновляем позицию мыши для плавного следования
                setDragState(prev => ({
                    ...prev,
                    currentPosition: { x: e.clientX, y: e.clientY }
                }));
    ...
    const handleMouseUp = useCallback(() => {
        if (dragState.isDragging && dragState.draggedAppointment && dragState.targetSlot) {
            ...
            // Проверяем только рабочие часы (накладывание разрешено)
    ...
    const handleAddAppointment = useCallback(() => {
        if (newAppointment.clientName.trim() && newAppointment.service && selectedEmployeeId && selectedTimeSlot) {
            ...
            // Убираем проверку на занятость слота - разрешаем накладывание
    ...
    const getAppointmentLayout = useCallback((employeeId: string) => {
        ...
        for (let i = 0; i < sortedAppointments.length; i++) {
            ...
            // Sort overlapping by duration (longer first) for proper layering
            ...
            // Calculate z-index: longer appointments get lower z-index (appear behind)
    ...
    const getOverlapColor = useCallback((appointment: Appointment, column: number, totalColumns: number) => {
        if (totalColumns === 1) {
            // Single appointment - use employee color
            const employee = employees.find(emp => emp.id === appointment.employeeId);
            return employee?.color || '#3B82F6';
        }

        // Multiple overlapping appointments - generate distinct colors
        const colors = [
            '#3B82F6', // blue
            '#10B981', // green  
            '#F59E0B', // amber
    ...
    const renderAppointmentBlock = (layoutInfo: {
        ...
        // Get color for this appointment based on overlap
                            ...
                            <div className="flex items-start justify-between">
                                ...
                                {width > 50 && (
                                    <div className="ml-1 pointer-events-none">
                                        <GripVertical size={10} className="text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="fixed top-4 right-4 bg-blue-600 text-white px-3 py-2 rounded-lg shadow-lg z-50 flex items-center gap-2">
                        {dragState.isDragging && (
                            <>
                                ...
                                {dragState.targetSlot ? 'Отпустите для перемещения' : 'Перетащите в нужную позицию'}
                            </>
                        )}
                        ...
                        >
                            <div className={`p-3 text-xs font-medium h-full flex flex-col justify-between relative overflow-hidden ${doesAppointmentFitWorkingHours(
                                ...
                                <div className="opacity-70 mt-auto relative z-10">
                                    ...
                                    <div className="text-xs opacity-60">({dragState.draggedAppointment?.duration} мин)</div>
                            ...
                            >
                                ...
                                <div className="relative">
                                    {timeSlots.map((slot, slotIndex) => {
                                        ...
                                        // Все рабочие часы доступны для клика (накладывание разрешено)
                                        return (
                                            <button
                                                key={`${employee.id}-${slot}`}
